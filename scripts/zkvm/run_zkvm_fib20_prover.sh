...
# Ensure fib_stdin.s exists in benchmarks (compile from C via Docker if needed)
...
WORKDIR="$(mktemp -d /tmp/zkvm_fib20_prover_XXXXXX)"
echo "[zee200 fib20 prover] WORKDIR=$WORKDIR"
