//! SHA256 benchmark program for SP1 zkVM.
//!
//! This program:
//!   - Reads the number of iterations from stdin
//!   - Computes SHA256 over a fixed message that matches the tight-zkvm benchmark
//!   - Repeats the hash `iterations` times
//!   - Checks the result against the expected digest and commits (iterations, ok)
...
fn nettle_sha256_compress(state: &mut [u32x; 8], input: &[u8; SHA256_BLOCK_SIZE]) {
    ...
    while i < 16 {
        for j in 0..8 {
            ...
            let d_val = data[i + j];
            match j {
                0 => round(&mut a, &mut b, &mut c, &mut d, &mut e, &mut f, &mut g, &mut h, k_val, d_val),
                1 => round(&mut h, &mut a, &mut b, &mut c, &mut d, &mut e, &mut f, &mut g, k_val, d_val),
                2 => round(&mut g, &mut h, &mut a, &mut b, &mut c, &mut d, &mut e, &mut f, k_val, d_val),
                3 => round(&mut f, &mut g, &mut h, &mut a, &mut b, &mut c, &mut d, &mut e, k_val, d_val),
                4 => round(&mut e, &mut f, &mut g, &mut h, &mut a, &mut b, &mut c, &mut d, k_val, d_val),
                5 => round(&mut d, &mut e, &mut f, &mut g, &mut h, &mut a, &mut b, &mut c, k_val, d_val),
                6 => round(&mut c, &mut d, &mut e, &mut f, &mut g, &mut h, &mut a, &mut b, k_val, d_val),
                7 => round(&mut b, &mut c, &mut d, &mut e, &mut f, &mut g, &mut h, &mut a, k_val, d_val),
                _ => {}
            }
        }
        i += 8;
    ...
    while i < 64 {
        for j in 0..16 {
            ...
            let k_val = K[idx];
            match j % 8 {
                0 => round(&mut a, &mut b, &mut c, &mut d, &mut e, &mut f, &mut g, &mut h, k_val, w),
                1 => round(&mut h, &mut a, &mut b, &mut c, &mut d, &mut e, &mut f, &mut g, k_val, w),
                2 => round(&mut g, &mut h, &mut a, &mut b, &mut c, &mut d, &mut e, &mut f, k_val, w),
                3 => round(&mut f, &mut g, &mut h, &mut a, &mut b, &mut c, &mut d, &mut e, k_val, w),
                4 => round(&mut e, &mut f, &mut g, &mut h, &mut a, &mut b, &mut c, &mut d, k_val, w),
                5 => round(&mut d, &mut e, &mut f, &mut g, &mut h, &mut a, &mut b, &mut c, k_val, w),
                6 => round(&mut c, &mut d, &mut e, &mut f, &mut g, &mut h, &mut a, &mut b, k_val, w),
                7 => round(&mut b, &mut c, &mut d, &mut e, &mut f, &mut g, &mut h, &mut a, k_val, w),
                _ => {}
            }
        }
        i += 16;
...
pub fn main() {
    // Read number of iterations from input (matches scanf in C version)
    let iterations: u32 = sp1_zkvm::io::read();

    let mut buffer = [0u8; SHA256_DIGEST_SIZE];

    // Same loop structure as C version: for each iteration,
    // reset buffer and context, hash MSG once, and store digest in buffer.
    ...
    // Check first 8 bytes of the digest against EXPECTED_HASH,
    // mirroring the tight-zkvm benchmark.
    let mut ok: u32 = 1;
    for i in 0..8 {
    ...
    // Commit the public values: iterations and correctness flag.
    sp1_zkvm::io::commit(&iterations);
