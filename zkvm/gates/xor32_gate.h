...
/**
 * 32-bit bitwise XOR gate implementation
 * Inherits from And32Gate and changes only the ROM offset and computation
 */
class Xor32Gate : public And32Gate {
public:
    ...
    void execute(std::size_t op_index, const BaseOp& op,
        ...
        for (int j = 0; j < 4; j++) {
            uint64_t idx = RomOffsets::XOR_OFFSET + (xval[j] * 256 + yval[j]);
