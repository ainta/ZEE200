...
constexpr std::array<std::uint32_t, 64> cleark = {
...
constexpr std::array<uint32_t, 16> clearfinal_chunk = {
...
void sha256(const Addr& input, Addr& output) {
  ...
  for (std::uint32_t i = 0; i < 16; ++i) {
    final_chunk[i] = clearfinal_chunk[i];
  }


  /* const auto right_rot = [](const U32& value, size_t count) { */
  ...
  /*       if (i > 0) { */
  /*         /1* Extend the first 16 words into the remaining 48 words w[16..63] of the message schedule std::array: *1/ */
  /*         const auto index1 = w[(j + 1u) & 0xfu]; */
  /*         const auto index14 = w[(j + 14u) & 0xfu]; */
  ...
  /*   0x5be0cd19 */
  /* }; */
  /* const auto h1 = add(h_init, compress(std::move(input), h_init)); */
