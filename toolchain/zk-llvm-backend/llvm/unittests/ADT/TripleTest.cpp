...
TEST(TripleTest, ParsedIDs) {
  ...
  T = Triple("zk-unknown-unknown");
  EXPECT_EQ(Triple::zk, T.getArch());
  EXPECT_EQ(Triple::UnknownVendor, T.getVendor());
  EXPECT_EQ(Triple::UnknownOS, T.getOS());
  EXPECT_EQ(Triple::UnknownEnvironment, T.getEnvironment());
...
TEST(TripleTest, BitWidthArchVariants) {
  ...
  T.setArch(Triple::zk);
  EXPECT_EQ(Triple::zk, T.get32BitArchVariant().getArch());
  EXPECT_EQ(Triple::UnknownArch, T.get64BitArchVariant().getArch());
}
...
TEST(TripleTest, EndianArchVariants) {
  ...
  T.setArch(Triple::zk);
  EXPECT_EQ(Triple::zk, T.getBigEndianArchVariant().getArch());
  EXPECT_EQ(Triple::UnknownArch, T.getLittleEndianArchVariant().getArch());
}
